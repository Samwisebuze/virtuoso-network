...
import { NetworkDocument } from '.'
import mongoSchema from './machine'
...
class NetworkClass extends mongoose.Model {
    ...
    */
    ...
    public static async getMachines({ networkId }): Promise<MachineDocument[]> {
        ...
        const network: NetworkDocument = await this.findById({ id: networkId })
    ...
    public static async get({ networkId }) {
        ...
        return this.findById({ id: networkId })
    }
    ...
    public static async getMachine({ networkId, machineId }) {
        ...
        const network = await this.findById({ id: networkId })
    ...
    public static async addMachine({
        ...
        if (!adjacentMachines) {
            adjacentMachines = new Array<string>()
        }
        ...
        const network = await this.findById({ id: networkId })
        const newMachine = network.machines.create(machine as MachineDocument)
    ...
    public static async editMachine({ networkId, machineId, machineDoc }) {
        if (!networkId || !machineId || !machineDoc) { throw new Error("Invalid Inputs") }

        const network = await this.findById({ id: networkId })

        network.machines.id(machineId).set(machineDoc)
...
mongoSchema.loadClass(NetworkClass)
